...
class ExtractTasks:
    ...
    def _extract_data_to_gcs(self, data_name, logical_date):
        ...
        df = pd.read_parquet(f"{CAB_DATA_BASE_URL}/{data_name}_tripdata_{year}-{month}.parquet")
        date_column_name = 'tpep_pickup_datetime' if data_name == 'yellow' else 'lpep_pickup_datetime'
